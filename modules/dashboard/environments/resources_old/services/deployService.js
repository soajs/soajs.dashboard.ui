	...
	function buildDeployForm(currentScope, context, $modalInstance, resource, action, settings, cb) {
						...
						if (recipes[i].recipe.buildOptions && recipes[i].recipe.buildOptions.env && Object.keys(recipes[i].recipe.buildOptions.env).length > 0) {
							for (var env in recipes[i].recipe.buildOptions.env) {
								if (recipes[i].recipe.buildOptions.env[env].type === 'userInput') {
									...
									if (context.formData.deployOptions.custom && context.formData.deployOptions.custom.env && context.formData.deployOptions.custom.env.hasOwnProperty(env)) {
										if (typeof context.formData.deployOptions.custom.env[env] === "string") {
											if (context.formData.deployOptions.custom.env[env] === "") {
												context.formData.deployOptions.custom.env[env] = angular.copy(recipes[i].recipe.buildOptions.env[env]);
											}
										} else {
		...
		context.buildComputedHostname = function (resourceName) {
							...
							selectedVMLayer.list.forEach((onevminstance) => {
								...
								onevminstance.ip.forEach((oneIP) => {
									if (oneIP.type === 'public' && !processed) {
										...
										serversArray.push({
											'host': oneIP.address,
											'port': (onevminstance.ports && onevminstance.ports[0]) ? onevminstance.ports[0].published : ''
										});
									}
								});
							});
			...
			if (context.form && context.form.entries && Array.isArray(context.form.entries) && context.form.entries.length > 0) {
				...
				let serversEntry = context.form.entries.find((oneEntry) => {
					return oneEntry.name === 'servers0';
				});
				if (serversEntry && serversEntry.entries && serversArray && serversArray.length > 0) {
					...
					serversArray.forEach((oneServer, index) => {
						...
						let hostField = oneEntryClone.entries.find((oneField) => {
							return oneField.name.includes('host');
						});
						let portField = oneEntryClone.entries.find((oneField) => {
							return oneField.name.includes('port');
						});
						let removeServerField = oneEntryClone.entries.find((oneField) => {
							return oneField.name.includes('removeserver');
						});
				...
				context.mainData.deploymentData.vmLayers[context.formData.deployOptions.deployConfig.vmConfiguration.vmLayer].list.forEach((oneInstance) => {
					if (oneInstance.securityGroup && Array.isArray(oneInstance.securityGroup) && oneInstance.securityGroup.length > 0) {
						oneInstance.securityGroup.forEach((oneSg) => {
							let matchingGroup = availableSecurityGroups.find((oneGroup) => {
								return oneGroup.v === oneSg;
							});
							if (!matchingGroup) {
							...
							if (oneLayerPort.access === 'allow' && oneLayerPort.direction === 'inbound') {
								if (oneLayerPort.isPublished === onePort.isPublished) {
									if (oneLayerPort.published.indexOf('-') !== -1) {
										let range = oneLayerPort.published.split('-').map((oneEntry) => {
											return parseInt(oneEntry.trim());
										});
		...
		context.listVmsApi = function (cb) {
			...
			function aggregateVMList() {
				...
				for (let i in context.mainData.deploymentData.vmLayers) {
					...
					if (context.mainData.deploymentData.vmLayers[i].list && context.mainData.deploymentData.vmLayers[i].executeCommand) {
		...
		environmentsConfig.form.addResource.data.categories.forEach((oneCategory) => {
