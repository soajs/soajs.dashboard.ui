"use strict";
var staticDeployCatalogApp = soajsApp.components;
staticDeployCatalogApp.controller('staticDeployCatalogCtrl', ['$scope', '$timeout', '$modal', '$compile', 'ngDataApi', 'injectFiles', '$cookies', 'statickubeServicesSrv', function ($scope, $timeout, $modal, $compile, ngDataApi, injectFiles, $cookies, statickubeServicesSrv) {
	...
	$scope.inspectItem = function (service) {
		statickubeServicesSrv.inspectItem($scope, service);
	};
	...
	$scope.getMetrics = function (pod) {
		...
		$modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				statickubeServicesSrv.autoRefreshMetrics($scope, $modalInstance, currentScope, pod);
			}
		});
	};
	...
	$scope.configure = function (service, version) {
		...
		let deployService = $modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				statickubeServicesSrv.configureDeployment($scope, currentScope, service, version, $modalInstance, function (err) {
					if (err) {
				...
				$scope.configure = function (service, version) {
					statickubeServicesSrv.saveConfiguration(service, version, $scope, currentScope, $modalInstance, function () {
						currentScope.listStaticCatalog();
					});
				};
				
				$scope.build = function (service, version) {
					statickubeServicesSrv.buildConfiguration(service, version, $scope, currentScope, $modalInstance, function () {
						currentScope.listStaticCatalog();
					});
				};
			}
...
staticDeployCatalogApp.filter('capitalizeFirst', function () {
