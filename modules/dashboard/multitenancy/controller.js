		...
		getSendDataFromServer($scope, ngDataApi, {
			...
			if (error) {
				...
				for (let x = result.length -1; x >= 0; x--) {
					if(result && result[x] && result[x].code && result[x].code.toUpperCase() === "DASHBOARD"){
						result.splice(x, 1);
						break;
					}
				}
				$scope.environments_codes = result;
...
multiTenantApp.controller('tenantConsoleCtrl', ['$scope', '$compile', '$timeout', '$modal', '$routeParams', 'ngDataApi', '$cookies', 'injectFiles', 'mtsc', function ($scope, $compile, $timeout, $modal, $routeParams, ngDataApi, $cookies, injectFiles, mtsc) {
	...
	$scope.editAppAcl = function (tId, appId) {
		$scope.$parent.go("/consoleTenants/" + tId + "/editConsoleAcl/" + appId);
	};
...
multiTenantApp.controller('tenantConsoleApplicationAcl', ['$scope', 'ngDataApi', '$routeParams', 'aclHelper', function ($scope, ngDataApi, $routeParams, aclHelper) {
	$scope.$parent.isUserLoggedIn();
	$scope.isInherited = false;
	$scope.currentApplication = {};
	$scope.allGroups = [];
	...
	$scope.fullAccess = true;
	//default operation
	if ($scope.access.tenant.list && $scope.access.product.listConsoleProducts && $scope.access.service.list && $scope.access.environment.list) {
		overlayLoading.show(function () {
			$scope.getEnvironments();
