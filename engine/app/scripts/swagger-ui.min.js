/*
 * Orange angular-swagger-ui - v0.6.1
 *
...
angular.module("swaggerUi", ["ng"]).directive("swaggerUi", ["$injector", function (a) {
	...
	function b(a, b) {
		return b.allOf && (angular.forEach(b.allOf, function (c) {
			angular.merge(b, q(a, c))
		}), delete b.allOf), b
	}
	
	function c(a, e, f) {
		var g, h, i, j;
		if (f = f || {}, e = b(a, e), e["default"] || e.example) g = e["default"] || e.example; else if (e.properties) {
			g = {};
			for (i in e.properties) j = e.properties[i], g[i] = c(a, j.schema || j, f)
		} else e.additionalProperties ? (h = q(a, e.additionalProperties), g = {string: c(a, h, f)}) : e.$ref ? (h = q(a, e), h ? (p[e.$ref] || f[e.$ref] || (f[e.$ref] = !0, p[e.$ref] = c(a, h, f)), g = p[e.$ref] || {}) : (console.warn("AngularSwaggerUI: schema not found", e.$ref), g = e.$ref)) : g = "array" === e.type ? [c(a, e.items, f)] : "object" === e.type ? {} : e.defaultValue || e.example || d(e);
		return g
	}
	...
	function h(a, c, d, f, g) {
		...
		if (c = b(a, c), c.properties) angular.forEach(c.properties, function (b) {
			...
			var k = c.schema || c, l = q(a, k), m = e(k);
			!l || "object" !== l.type && "array" !== l.type ? l && (c = angular.merge(c, l), delete c.$ref) : (j[m] = l, d[m] = t++, angular.merge(j, h(a, l, d, m, g)))
		} else "array" === c.type ? i(a, c.items, j, d, g) : c.additionalProperties && i(a, c.additionalProperties, j, d, g);
		return c.discriminator && angular.forEach(a.definitions, function (b, c) {
			b.allOf && angular.forEach(b.allOf, function (i) {
				i.$ref && f === e(i) && (b.parentModel = f, j[c] = b, d[c] = t++, angular.merge(j, h(a, b, d, c, g)))
			})
		}), j
	}
	...
	function j(a, b, c) {
		var d;
		return a.properties && (d = g(), a.modelName = d, c[d] = t++, b[d] = a), d
	}
	
	function k(a, b, c) {
		var d = a + "-model-" + c;
		return ['<a class="model-link type" onclick="swaggerlink(\'', d, "')\">", c, "</a>"].join("")
	}
	
	function l(c, d, e, h, i, j) {
		var k = ['<div class="model" id="', j + "-model-" + e, '">'];
		if (d = b(c, d), d.properties) {
			k.push("<div><strong>", e), d.parentModel && k.push("</strong> extends <strong>", d.parentModel), k.push(" {</strong></div>");
			var l = !1;
			angular.forEach(d.properties, function (b, c) {
				l = !0, k.push('<div class="pad"><strong>', c, '</strong> (<span class="type">'), k.push(m(b, h, i, j)), k.push("</span>"), f(d, c) || k.push(", ", "<em>" + a.translate("modelOptional") + "</em>"), k.push(")"), b.description && k.push(": ", b.description);
				var e = b["enum"] || b.items && b.items["enum"];
				e && k.push(", Enum = ", angular.toJson(e).replace(/,/g, ", ")), k.push(",</div>")
			}), l && (k.pop(), k.push("</div>")), k.push("<div><strong>}</strong></div>")
		} else "array" === d.type || d.additionalProperties ? k.push(m(d, h, i, j)) : "object" === d.type ? k.push("<strong>", e || g(), " {<br>}</strong>") : d.type && k.push("<strong>", r(d), "</strong>");
		return k.join("")
	}
	
	function m(a, b, c, d) {
		var f, g = [];
		return a.modelName ? g.push(k(d, c[a.modelName], a.modelName)) : a.schema || a.$ref ? (f = e(a.schema || a), g.push(k(d, c[f], f))) : "array" === a.type ? (g.push("Array["), g.push(m(a.items, b, c, d)), g.push("]")) : a.properties ? g.push(k(d, c[a.modelName], f)) : a.additionalProperties ? (g.push("Map&lt;string, "), g.push(m(a.additionalProperties, b, c, d)), g.push("&gt;")) : g.push(r(a)), g.join("")
	}
	...
	var o = "InlineModel", p = {}, q = this.resolveReference = function (a, c) {
		var d = c;
		if (c.$ref) {
	...
	this.generateModel = function (a, c, d) {
		var e = [], f = {}, i = {};
		try {
			c = b(a, c), c.properties ? (i[g()] = c, i = angular.merge(i, h(a, c, f))) : i = h(a, c, f), c.$ref || c.properties || e.push("<strong>", m(c, i, f, d), "</strong><br><br>"), angular.forEach(i, function (b, c) {
				e.push(l(a, b, c, i, f, d))
			})
		} catch (j) {
			console.error("AngularSwaggerUI: failed to generate model", c, j), e = ["failed to generate model"]
		}
		return n(), e.join("")
	};
	var t = 0;
	...
	function f(a, b) {
		var d = {}, e = {}, f = [], j = b.openApiSpec.info, k = c.hash(), l = "application/json",
			m = !b.openApiSpec.tags;
		r = 0, s = 0, g(b.openApiSpec, b.url, j, l), h(b.openApiSpec, f, d), i(b.openApiSpec, f, e, d, l, k), n(f, k, m), b.ui = {
			infos: j,
			form: e,
			resources: f
		}, a.resolve(!0)
