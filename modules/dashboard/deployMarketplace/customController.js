"use strict";
var customDeployCatalogApp = soajsApp.components;
customDeployCatalogApp.controller('customDeployCatalogCtrl', ['$scope', '$timeout', '$modal', '$compile', 'ngDataApi', 'injectFiles', '$cookies', 'customkubeServicesSrv', function ($scope, $timeout, $modal, $compile, ngDataApi, injectFiles, $cookies, customkubeServicesSrv) {
	...
	$scope.inspectItem = function (service) {
		customkubeServicesSrv.inspectItem($scope, service);
	};
	
	$scope.getLogs = function (pod) {
		customkubeServicesSrv.getLogs($scope, pod);
	};
	...
	$scope.execCommand = function (pod, service, version) {
		...
		$modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				customkubeServicesSrv.execCommand($scope, $modalInstance, currentScope, pod, service, version);
			}
		});
	};
	...
	$scope.execCommandPods = function (service, version) {
		...
		$modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				customkubeServicesSrv.execCommands($scope, $modalInstance, currentScope, service, version);
			}
		});
	};
	...
	$scope.getMetrics = function (pod) {
		...
		$modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				customkubeServicesSrv.autoRefreshMetrics($scope, $modalInstance, currentScope, pod);
			}
		});
	};
	...
	$scope.configure = function (service, version) {
		...
		let deployService = $modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				customkubeServicesSrv.configureDeployment($scope, currentScope, service, version, $modalInstance, function (err) {
					if (err) {
				...
				$scope.configure = function (service, version) {
					customkubeServicesSrv.saveConfiguration(service, version, $scope, currentScope, $modalInstance, function () {
						currentScope.listCustomCatalog();
					});
				};
				
				$scope.build = function (service, version) {
					customkubeServicesSrv.buildConfiguration(service, version, $scope, currentScope, $modalInstance, function () {
						currentScope.listCustomCatalog();
					});
				};
			}
		...
		if (!$scope.configuration) {
			...
			let deployService = $modal.open({
				...
				controller: function ($scope, $modalInstance) {
					...
					customkubeServicesSrv.reConfigureDeployment($scope, currentScope, service, version, $modalInstance, function (err) {
						if (err) {
							$scope.$parent.displayAlert('danger', err.message);
						}
					});
					...
					$scope.redeploy = function (service, version) {
						customkubeServicesSrv.redeploy(service, version, $scope, currentScope, $modalInstance);
					};
				}
			});
		}
...
customDeployCatalogApp.filter('capitalizeFirst', function () {
