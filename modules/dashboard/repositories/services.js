...
repoService.service('repoSrv', ['ngDataApi', '$timeout', '$modal', '$cookies', '$window', '$compile', 'detectBrowser', function (ngDataApi, $timeout, $modal, $cookies, $window, $compile, detectBrowser) {
	...
	function configureRepo(currentScope, oneRepo, gitAccounts, config) {
		currentScope.configureRepoEditor = true;
		if (oneRepo.source.length > 1) {
			currentScope.gitAccounts = gitAccounts;
			currentScope.applyAccount = function (owner) {
				if (owner) {
					currentScope.gitAccount = owner;
					currentScope.showConfiguration = true;
					configure(currentScope, oneRepo, config);
				}
			}
		} else {
			currentScope.gitAccounts = gitAccounts;
			gitAccounts.forEach((one) => {
				if (one.owner === oneRepo.source[0].name) {
					currentScope.gitAccount = one;
				}
			});
			configure(currentScope, oneRepo, config);
		}
		
		function configure(currentScope, oneRepo, config) {
			...
			currentScope.showCIConfigForm = function (oneProvider) {
				currentScope.ciRepoName = oneRepo.repository;
				currentScope.activatedRepo = false;
								...
								if (currentScope.access.getCIProviders) {
									getProviderRecipes(currentScope, {
										...
										if (currentScope.access.getCIRepoCustomRecipe) {
											getRepoRecipeFromBranch(currentScope, currentScope.gitAccount, oneProvider, oneRepo, providerRecipes, function (branchInput) {
												formConfig.entries.push({
													"type": "accordion",
													"name": "repoRecipe",
												...
												buildForm(currentScope, null, options, function (err, res) {
			...
			if (!currentScope.access.getCIAccountInfo) {
				...
				getCIRecipe(currentScope, currentScope.gitAccount, function (ciProviders) {
	...
	function getRepoRecipeFromBranch(currentScope, gitAccount, provider, repo, providerRecipes, cb) {
		getServiceBranches(currentScope, {
			...
			gitBranchResponse.forEach(function (oneBranch) {
	...
	function getServiceBranches(currentScope, opts, cb) {
		...
		getSendDataFromServer(currentScope, ngDataApi, {
			method: 'get',
			routeName: '/repositories/git/branches',
			params: {
				'id': opts.repo._id
			}
		}, function (error, response) {
