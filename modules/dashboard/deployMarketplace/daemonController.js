"use strict";
let daemoonDeployCatalogApp = soajsApp.components;
daemoonDeployCatalogApp.controller('daemonDeployCatalogCtrl', ['$scope', '$timeout', '$modal', '$compile', 'ngDataApi', 'injectFiles', '$cookies', 'daemonkubeServicesSrv', function ($scope, $timeout, $modal, $compile, ngDataApi, injectFiles, $cookies, daemonkubeServicesSrv) {
	...
	$scope.inspectItem = function (service) {
		daemonkubeServicesSrv.inspectItem($scope, service);
	};
	...
	$scope.getMetrics = function (pod) {
		...
		$modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				daemonkubeServicesSrv.autoRefreshMetrics($scope, $modalInstance, currentScope, pod);
			}
		});
	};
	...
	$scope.configure = function (service, version) {
		...
		let deployService = $modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				daemonkubeServicesSrv.configureDeployment($scope, currentScope, service, version, $modalInstance, function (err) {
					if (err) {
				...
				$scope.configure = function (service, version) {
					daemonkubeServicesSrv.saveConfiguration(service, version, $scope, currentScope, $modalInstance, function () {
						currentScope.listApiCatalog();
					});
				};
				
				$scope.build = function (service, version) {
					daemonkubeServicesSrv.buildConfiguration(service, version, $scope, currentScope, $modalInstance, function () {
						currentScope.listApiCatalog();
					});
				};
			}
...
daemoonDeployCatalogApp.filter('capitalizeFirst', function () {
