"use strict";
let resourceDeployCatalogApp = soajsApp.components;
resourceDeployCatalogApp.controller('resourceDeployCatalogCtrl', ['$scope', '$timeout', '$modal', '$compile', 'ngDataApi', 'injectFiles', '$cookies', 'resourcekubeServicesSrv', function ($scope, $timeout, $modal, $compile, ngDataApi, injectFiles, $cookies, resourcekubeServicesSrv) {
	...
	$scope.inspectItem = function (service) {
		resourcekubeServicesSrv.inspectItem($scope, service);
	};
	
	$scope.getLogs = function (pod) {
		resourcekubeServicesSrv.getLogs($scope, pod);
	};
	...
	$scope.execCommand = function (pod) {
		...
		$modal.open({
			...
			controller: function ($scope, $modalInstance) {
				resourcekubeServicesSrv.execCommand($scope, $modalInstance, currentScope, pod);
			}
		});
	};
	...
	$scope.getMetrics = function (pod) {
		...
		$modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				resourcekubeServicesSrv.autoRefreshMetrics($scope, $modalInstance, currentScope, pod);
			}
		});
	};
	...
	$scope.configure = function (service, version) {
		...
		let deployService = $modal.open({
			...
			controller: function ($scope, $modalInstance) {
				...
				resourcekubeServicesSrv.configureDeployment($scope, currentScope, service, version, $modalInstance, function (err) {
					if (err) {
				...
				$scope.configure = function (service, version) {
					resourcekubeServicesSrv.saveConfiguration(service, version, $scope, currentScope, $modalInstance, function () {
						currentScope.listResourceCatalog();
					});
				};
				
				$scope.build = function (service, version) {
					resourcekubeServicesSrv.buildConfiguration(service, version, $scope, currentScope, $modalInstance, function () {
						currentScope.listResourceCatalog();
					});
				};
			}
...
resourceDeployCatalogApp.filter('capitalizeFirst', function () {
