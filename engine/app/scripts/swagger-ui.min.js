 ...
 */
"use strict";
angular.module("swaggerUi", ["ng"]).directive("swaggerUi", ["$injector", function (a) {
	return {
		restrict: "A",
		controller: "swaggerUiController",
		templateUrl: function (a, b) {
			return b.templateUrl || "templates/swagger-ui.html"
		},
		scope: {
			url: "=?",
			parser: "@?",
		...
		link: function (b) {
			b.trustedSources || a.has("$sanitize") || console.warn("AngularSwaggerUI: You must use ngSanitize OR set trusted-sources=true as directive param if OpenApi specifications are loaded from trusted sources"), void 0 === b.validatorUrl && (b.validatorUrl = "http://online.swagger.io/validator"), "undefined" == typeof b.download && (b.download = null)
		}
	}
}]), angular.module("swaggerUi").controller("swaggerUiController", ["$scope", "$window", "$location", "$anchorScroll", "$timeout", "$sce", "$q", "swaggerClient", "swaggerModules", "swaggerTranslator", "swaggerLoader", "swaggerModel", function (a, b, c, d, e, f, g, h, i, j, k, l) {
	function m(b, c) {
		a.loading = !1, l.clearCache(), i.execute(i.PARSE, c).then(function (a) {
			a ? n(c) : o({code: 415, message: j.translate("errorNoParserFound", {type: c.contentType})})
		})["catch"](o)
	}
	
	function n(b) {
		i.execute(i.BEFORE_DISPLAY, b).then(function () {
			v = b.openApiSpec, a.ui = b.ui, a.permalinks && e(function () {
				d()
			}, 200)
		})["catch"](o)
	}
	...
	function o(b) {
		console.error("AngularSwaggerUI", b), a.loading = !1, "function" == typeof a.errorHandler && a.errorHandler(b.message, b.code)
	}
	
	function p() {
		return a.$watch("input", function (b) {
			r(), b && (a.loading = !0, m(null, {
				openApiSpec: angular.copy(b),
				parser: a.parser || a.inputType || "json",
				contentType: "application/" + (a.inputType || "json")
			}))
		})
	}
	...
	function q(b) {
		return a.$watch(b, function (b) {
			if (r(), b && "" !== b) {
				if (a.loading, a.validatorUrl && 0 !== b.indexOf("http")) return void (a.url = t(b));
				a.loading = !0;
				var c = {url: b, parser: a.parser || "auto", trustedSources: a.trustedSources};
				k.get(c).then(function () {
					return i.execute(i.BEFORE_PARSE, c)
				}).then(function () {
					m(b, c)
				})["catch"](o)
			}
	...
	function r() {
		a.ui = {form: {}, infos: {}, resources: []}, v = null
	}
	
	function s() {
		var b;
		a.$watch("inputType", function (c) {
			switch (r(), b && b(), c) {
				case"json":
				case"yaml":
					a.validatorUrl = !1, b = p();
					break;
				case"url":
					b = q("input");
					break;
	...
	function u(a) {
		var b, c, d = 0, e = {}, f = a.security || [];
		if (v) for (; d < f.length; d++) {
			b = f[d];
			for (c in b) e[c] = v.securityDefinitions[c]
		}
		return e
	}
	
	var v;
	s(), a.expand = function (a, b) {
		a.open = !0;
		for (var c = 0, d = a.operations, e = d.length; c < e; c++) d[c].open = b
	}, b.swaggerlink = a.permalink = function (b) {
		a.permalinks && e(function () {
			c.hash(b), d(b)
		}, 50)
	}, a.submitExplorer = function (b) {
		b.loading = !0, h.send(v, b, a.ui.form[b.id], u(b)).then(function (a) {
			b.loading = !1, b.explorerResult = a
		})
	}, a.auth = function (a) {
		i.execute(i.AUTH, {securityDefinitions: u(a)})["catch"](o)
	}, a.authValid = function (a) {
		var b, c = u(a);
		for (b in c) if (!c[b].valid) return !1;
		return !0
	}, a.getModel = function (a, b, c) {
		var d = b + "-" + c;
		return a.models = a.models || {}, !a.models[d] && a.schema && (a.models[d] = f.trustAsHtml(l.generateModel(v, a.schema, d))), a.models[d]
	}, a.getSample = function (a, b) {
		return a.samples = a.samples || {}, b && !a.samples[b] && a.schema && (a.samples[b] = l[b.indexOf("/xml") >= 0 ? "generateSampleXml" : "generateSampleJson"](v, a.schema, a.examples && a.examples[b])), a.samples[b]
	}
}]), angular.module("swaggerUi").directive("fileInput", function () {
	return {
...
}), angular.module("swaggerUi").directive("selectOnDbClick", ["$window", function (a) {
	return {
		restrict: "A", link: function (b, c) {
			c.bind("dblclick", function () {
				var b = a.getSelection(), d = document.createRange();
				d.selectNodeContents(c[0]), b.removeAllRanges(), b.addRange(d)
			})
		}
	}
}]), angular.module("swaggerUi").service("swaggerClient", ["$q", "$http", "$httpParamSerializer", "swaggerModules", function (a, b, c, d) {
	function e(a, b) {
		var c = "", d = b.data, e = b.config;
		if (e.params) {
			var f = [];
	...
	this.send = function (f, g, h, i) {
		var j, k = a.defer(), l = {}, m = {}, n = g.path, o = "application/x-www-form-urlencoded" === h.contentType;
		n.match(/.*\?.*({(\?|&).*})/) ? n = n.split("?")[0] : n.match(/.*({(\?).*})/) && (n = n.split("{?")[0]);
		for (var p = 0, q = g.parameters || [], r = q.length; p < r; p++) {
			var s = q[p], t = h[s.name];
			switch (s["in"]) {
				case"query":
					t && (l[s.name] = t);
					break;
				case"path":
					n = n.replace(new RegExp("{" + s.name + "[^}]*}"), encodeURIComponent(t));
					break;
				case"header":
					t && (m[s.name] = t);
					break;
				case"formData":
					j = j || (o ? {} : new FormData), t && ("file" === s.type && (h.contentType = void 0), o ? j[s.name] = t : j.append(s.name, t));
					break;
				case"body":
					j = j || t
			}
		...
		angular.forEach(i, function (a) {
			if (a.valid) switch (a.type) {
				case"apiKey":
					...
					break;
				default:
					a.tokenType && a.accessToken && (m.Authorization = a.tokenType + " " + a.accessToken)
			}
		}), m.Accept = h.responseType, m["Content-Type"] = j ? h.contentType : "text/plain";
		var u = f.basePath || "",
			v = [f.schemes[0], "://", f.host, u.length > 0 && "/" === u.substring(u.length - 1) ? u.slice(0, -1) : u].join(""),
			w = {method: g.httpMethod, url: v + n, headers: m, data: o ? c(j) : j, params: l}, x = function (a) {
				var b = {data: a.data, status: a.status, headers: a.headers, config: a.config};
				d.execute(d.AFTER_EXPLORER_LOAD, b).then(function () {
					e(k, b)
				})
			};
		return d.execute(d.BEFORE_EXPLORER_LOAD, w).then(function () {
			b(w).then(x)["catch"](x)
		}), k.promise
	}
}]), angular.module("swaggerUi").provider("swaggerTranslator", function () {
	var a = "en", b = {};
	this.setLanguage = function (b) {
		return a = b, this
	}, this.addTranslations = function (a, c) {
		var d = b[a] = b[a] || {};
		return angular.merge(d, c), this
	}, this.$get = ["$rootScope", "$interpolate", function (c, d) {
		return {
			useLanguage: function (d) {
				"undefined" == typeof b[d] && console.error("AngularSwaggerUI: No translations found for language " + d), a = d, c.$emit("swaggerTranslateLangChanged")
			}, translate: function (c, e) {
				return a && b && b[a] && b[a][c] ? d(b[a][c])(e) : c
			}, language: function () {
				return a
			}
		}
	}]
}).directive("swaggerTranslate", ["$rootScope", "$parse", "swaggerTranslator", function (a, b, c) {
	return {
		restrict: "A", link: function (d, e, f) {
			function g() {
				var a;
				f.swaggerTranslateValue && (a = b(f.swaggerTranslateValue)(d.$parent)), e.text(c.translate(f.swaggerTranslate, a))
			}
			
			var h = a.$on("swaggerTranslateLangChanged", function () {
				g()
			});
			d.$on("$destroy", h), g()
		}
	}
}]).filter("swaggerTranslate", ["swaggerTranslator", function (a) {
	var b = function (b, c) {
		return a.translate(b, c)
	};
	return b.$stateful = !0, b
}]), angular.module("swaggerUi").service("swaggerLoader", ["$q", "$http", "swaggerModules", function (a, b, c) {
	this.get = function (d) {
		var e = a.defer(), f = {method: "GET", url: d.url};
		return c.execute(c.BEFORE_LOAD, f).then(function () {
			return b(f)
		}).then(function (a) {
			return d.openApiSpec = a.data, d.contentType = (a.headers()["content-type"] || "application/json").split(";")[0], c.execute(c.AFTER_LOAD, d)
		}).then(function () {
			e.resolve(d)
		})["catch"](function (a) {
			a.status && (a = {code: a.status, message: a.data}), e.reject(a)
		}), e.promise
	}
}]), angular.module("swaggerUi").service("swaggerModel", ["swaggerTranslator", function (a) {
	function b(a, b) {
		return b.allOf && (b = angular.copy(b), angular.forEach(b.allOf, function (c) {
			var d = r(a, c);
			d.discriminator || angular.merge(b, d)
		}), delete b.allOf), b
	}
	
	function c(a, e, f) {
		var g, h, i, j;
		if (f = f || {}, e = b(a, e), e.parentModelsRef && angular.forEach(e.parentModelsRef, function (b) {
			var c = r(a, b);
			angular.merge(e, c)
		}), e["default"] || e.example) g = e["default"] || e.example; else if (e.properties) {
			g = {};
			for (i in e.properties) j = e.properties[i], g[i] = c(a, j.schema || j, f)
		} else e.additionalProperties ? (h = r(a, e.additionalProperties), g = {string: c(a, h, f)}) : e.$ref ? (h = r(a, e), h ? (q[e.$ref] || f[e.$ref] || (f[e.$ref] = !0, q[e.$ref] = c(a, h, f)), g = q[e.$ref] || {}) : (console.warn("AngularSwaggerUI: schema not found", e.$ref), g = e.$ref)) : g = "array" === e.type ? [c(a, e.items, f)] : "object" === e.type ? {} : e.defaultValue || e.example || d(e);
		return g
	}
	...
	function d(a) {
		...
		switch (c) {
			...
			case"binary":
			case"password":
				b = "string", a["enum"] && a["enum"].length > 0 && (b = a["enum"][0]);
				break;
			case"date":
	...
	function f(a, b) {
		return a.required && a.required.indexOf(b) !== -1
	}
	...
	function h(a, c, d, f, g) {
		var k = {};
		if (f) {
			if (g = g || {}, g[f]) return k;
			g[f] = !0
		}
		if (c = b(a, c), c.properties) angular.forEach(c.properties, function (b) {
			j(a, b, k, d, g)
		}); else if (c.schema || c.$ref) {
			var l = c.schema || c, m = r(a, l), n = e(l);
			!m || m.type && "object" !== m.type && "array" !== m.type || (m = b(a, m), k[n] = m, d[n] = u++, angular.merge(k, h(a, m, d, n, g)))
		} else "array" === c.type ? j(a, c.items, k, d, g) : c.additionalProperties && j(a, c.additionalProperties, k, d, g);
		if (c.subModelsRef) {
			for (var o = c.subModelsRef.map(e), p = !1, q = Object.keys(d), s = 0; s < o.length; s++) if (q.indexOf(o[s]) > -1) {
				p = !0;
				break
			}
			p || i(a, c.subModelsRef, k, d, g)
		}
		return c.parentModelsRef && i(a, c.parentModelsRef, k, d, g), k
	}
	
	function i(a, b, c, d, f) {
		angular.forEach(b, function (b) {
			var g = r(a, b), i = e(b);
			c[i] = g, d[i] = u++, angular.merge(c, h(a, g, d, i, f))
		})
	}
	
	function j(a, b, c, d, e) {
		var f = k(b, c, d);
		angular.merge(c, h(a, b, d, f, e))
	}
	
	function k(a, b, c) {
		var d;
		return a.properties && (d = g(), a.modelName = d, c[d] = u++, b[d] = a), d
	}
	
	function l(a, b) {
		var c = a + "-model-" + b;
		return ['<a class="model-link type" onclick="swaggerlink(\'', c, "')\">", b, "</a>"].join("")
	}
	
	function m(c, d, h, i, j, k) {
		var m = ['<div class="model" id="', k + "-model-" + h, '">'];
		if (d = b(c, d), d.properties) {
			m.push("<div><strong>", h), d.parentModelsRef && (m.push("</strong> extends <strong>"), angular.forEach(d.parentModelsRef, function (a) {
				m.push(l(k, e(a)), " ")
			}), m.pop()), m.push(" {</strong></div>");
			var o = !1;
			angular.forEach(d.properties, function (b, e) {
				if (o = !0, !b.type && b.$ref) {
					var g = r(c, b);
					"object" !== g.type && "array" !== g.type && (b = g)
				}
				m.push('<div class="pad"><strong>', e, '</strong> (<span class="type">'), m.push(n(b, i, j, k)), m.push("</span>"), f(d, e) || m.push(", ", "<em>" + a.translate("modelOptional") + "</em>"), m.push(")"), b.description && m.push(": ", b.description);
				var h = b["enum"] || b.items && b.items["enum"];
				h && m.push(", Enum = ", angular.toJson(h).replace(/,/g, ", ")), m.push(",</div>")
			}), o && (m.pop(), m.push("</div>")), m.push("<div><strong>}</strong></div>")
		} else "array" === d.type || d.additionalProperties ? m.push(n(d, i, j, k)) : "object" === d.type ? m.push("<strong>", h || g(), " {<br>}</strong>") : d.type && m.push("<strong>", s(d), "</strong>");
		return m.join("")
	}
	
	function n(a, b, c, d) {
		var f, g = [];
		return a.modelName ? g.push(l(d, a.modelName)) : a.schema || a.$ref ? (f = e(a.schema || a), g.push(l(d, f))) : "array" === a.type ? (g.push("Array["), g.push(n(a.items, b, c, d)), g.push("]")) : a.properties ? g.push(l(d, f)) : a.additionalProperties ? (g.push("Map&lt;string, "), g.push(n(a.additionalProperties, b, c, d)), g.push("&gt;")) : g.push(s(a)), g.join("")
	}
	...
	var p = "InlineModel", q = {}, r = this.resolveReference = function (a, c) {
		var d = c;
		if (c.$ref) {
			var e = c.$ref.replace("#/", "").split("/");
			d = a;
			for (var f = 0, g = e.length; f < g; f++) d = d[e[f]]
		}
		return d = b(a, d), angular.copy(d)
	}, s = this.getType = function (a) {
		var b = a.format;
		switch (b) {
			case"int32":
				b = a.type;
	...
	this.generateSampleJson = function (a, b, d) {
		var e, f;
		try {
			e = d || c(a, b), e && (f = angular.toJson(e, !0))
		} catch (g) {
			console.error("AngularSwaggerUI: failed to generate sample json", b, g), f = "failed to generate sample json"
		}
		return f
	}, this.generateSampleXml = function (a, b, c) {
		return '<?xml version="1.0" encoding="UTF-8"?>\n<!-- XML example cannot be generated -->'
	};
	var t = 1;
	this.resolveInheritance = function (a) {
		angular.forEach(a.definitions, function (b, c) {
			b.discriminator && (b.subModelsRef = [], angular.forEach(a.definitions, function (a, d) {
				b !== a && a.allOf && angular.forEach(a.allOf, function (f) {
					f.$ref && c === e(f) && (a.parentModelsRef = a.parentModelsRef || [], a.parentModelsRef.push({$ref: "#/definitions/" + c}), b.subModelsRef.push({$ref: "#/definitions/" + d}))
				})
			}))
		})
	}, this.generateModel = function (a, c, d) {
		var e, f = [], i = {}, j = {};
		try {
			c = b(a, c), c.properties ? (j[g()] = c, j = angular.merge(j, h(a, c, i))) : j = h(a, c, i), e = r(a, c), e.properties || f.push("<strong>", n(e, j, i, d), "</strong><br><br>"), angular.forEach(j, function (b, c) {
				f.push(m(a, b, c, j, i, d))
			})
		} catch (k) {
			console.error("AngularSwaggerUI: failed to generate model", c, k), f = ["failed to generate model"]
		}
		return o(), f.join("")
	};
	var u = 0;
	...
	function b(a, c, d, e) {
		var f = c.shift();
		f ? f.execute(d).then(function (f) {
			e = e || f, b(a, c, d, e)
		})["catch"](a.reject) : a.resolve(e)
	}
	
	var c = {};
	this.AUTH = "AUTH", this.BEFORE_LOAD = "BEFORE_LOAD", this.AFTER_LOAD = "AFTER_LOAD", this.BEFORE_PARSE = "BEFORE_PARSE", this.PARSE = "PARSE", this.BEFORE_DISPLAY = "BEFORE_DISPLAY", this.BEFORE_EXPLORER_LOAD = "BEFORE_EXPLORER_LOAD", this.AFTER_EXPLORER_LOAD = "AFTER_EXPLORER_LOAD", this.BEFORE_CONVERT = "BEFORE_CONVERT", this.add = function (a, b, d) {
		c[a] || (c[a] = []), d || (d = 1), b.swaggerModulePriority = d, c[a].indexOf(b) < 0 && c[a].push(b), c[a].sort(function (a, b) {
			return a.swaggerModulePriority > b.swaggerModulePriority ? -1 : a.swaggerModulePriority < b.swaggerModulePriority ? 1 : 0
		})
	}, this.execute = function (d, e) {
		var f = a.defer(), g = c[d] || [];
		return angular.isObject(e) || console.warn("AngularSwaggerUI: module execution argument should be an object!"), b(f, [].concat(g), e), f.promise
	}
}]), angular.module("swaggerUi").service("swaggerParser", ["$q", "$sce", "$location", "swaggerModel", "swaggerTranslator", function (a, b, c, d, e) {
	function f(a, b) {
		var e = {}, f = {}, j = [], k = b.openApiSpec.info, l = c.hash(), m = "application/json",
			o = !b.openApiSpec.tags;
		r = 0, s = 0, g(b.openApiSpec, b.url, k, m), h(b.openApiSpec, j, e), i(b.openApiSpec, j, f, e, m, l), d.resolveInheritance(b.openApiSpec), n(j, l, o), b.ui = {
			infos: k,
			form: f,
			resources: j
		}, a.resolve(!0)
	...
	function g(a, b, c, d) {
		...
		a.schemes = [a.schemes && a.schemes[0] || e.protocol.replace(":", "")], a.host = a.host || e.host, a.consumes = a.consumes || [d], a.produces = a.produces || [d], c.scheme = a.schemes[0], c.schemes = a.schemes, c.basePath = a.basePath, c.host = a.host, c.description = o(c.description), c.externalDocs = a.externalDocs, c.externalDocs && (c.externalDocs.description = o(c.externalDocs.description))
	}
	
	function h(a, b, c) {
		var d, e, f;
		if (a.tags) for (d = 0, e = a.tags.length; d < e; d++) f = a.tags[d], b.push(f), c[f.name] = d; else b.push({
			name: "default",
			open: !0
		}), c["default"] = 0
	}
	...
	function n(a, b, c) {
		var d, e, f;
		for (d = 0; d < a.length; d++) e = a[d], f = a[d].operations, e.open = e.open || b === e.name || b === e.name + "*", (!f || f && 0 === f.length) && a.splice(d--, 1);
	...
	function o(a) {
		var c = a;
		return "string" == typeof a && q && (c = b.trustAsHtml(p(a))), c
	}
	
	function p(a) {
		return a.replace(/&/g, "&amp;").replace(/<([^\/a-zA-Z])/g, "&lt;$1").replace(/"/g, "&quot;").replace(/'/g, "&#039;")
	}
	
	var q, r, s, eA, t = ["get", "put", "post", "delete", "options", "head", "patch", "trace"];
	...
	this.execute = function (b) {
		eA = b;
		var c = a.defer();
		if (b.openApiSpec && ("2.0" === b.openApiSpec.swagger || b.openApiSpec.openapi) && ("json" === b.parser || "auto" === b.parser && "application/json" === b.contentType)) {
			q = b.trustedSources;
			try {
				f(c, b)
			} catch (d) {
				c.reject({code: 500, message: e.translate("errorParseFailed", d)})
			}
		} else c.resolve(!1);
		return c.promise
	}
}]).run(["swaggerModules", "swaggerParser", function (a, b) {
	a.add(a.PARSE, b, 1)
}]), angular.module("swaggerUi").config(["swaggerTranslatorProvider", function (a) {
	a.addTranslations("en", {
