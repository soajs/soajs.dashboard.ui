...
deployServices.service('deploymentSrv', ['ngDataApi', '$timeout', '$modal', '$localStorage', '$window', function (ngDataApi, $timeout, $modal, $localStorage, $window) {
	...
	/*function switchDriver (driver) {
		...
		switch (driver) {
			case 'previous':
				if(currentScope.form.formData.deployment.previousEnvironment){
				...
				currentScope.platforms.manual = false;
			...
			default:
				...
				currentScope.platforms.docker = false;
				currentScope.platforms.kubernetes = false;
				currentScope.platforms.manual = true;
				break;
		}
	}*/
	...
	function handleFormData(currentScope, formData, advancedMode){
		...
		// 	currentScope.wizard.deploy = angular.copy(formData);
		//
		// 	currentScope.lastStep = 2;
		// 	if(advancedMode){
		// 		currentScope.Step3();
		// 	}
		// 	else if(currentScope.portalDeployment){
		// 		currentScope.Step21();
		// 	}
		// 	else{
		// 		overview.run($scope);
		...
		if(!currentScope.wizard.template.content || Object.keys(currentScope.wizard.template.content).length === 0){
			options.actions.push({
				...
				'action': function(formData){
					handleFormData(currentScope, formData, false);
				}
			});
		}
		else{
			options.actions.push({
				...
				'btn': 'primary',
				'action': function (formData) {
					handleFormData(currentScope, formData, true);
				}
			});
		}
